...
# filter out mutations within exons --> already done in readtbl.R
# icgc_snps<- icgc_snps %>%
...
cis_snpcount <- cis_snps %>% 
    ...
    ungroup()
write.csv(cis_snpcount, paste0(cistype,"_snp_count.csv") , row.names = FALSE, quote=FALSE)
...
snps_in_cis <- cis_snps %>%
  ...
  select(chromosome = chromosome.y,
         ...
         ref,mut) %>%
  distinct()
write.csv(snps_in_cis, paste0("snps_in_",cistype,".csv") , row.names = FALSE, quote=FALSE)
...
ggsave(paste0(cistype,"_tri_mutdist.png"))
dev.off()
...
write.csv(trifreq_mut_rate, paste0(cistype,"_tri_mutrate.csv") , row.names = FALSE, quote=FALSE)
...
ggsave(paste0(cistype,"_tri_bg.png"))
dev.off()
...
ggsave(paste0(cistype,"_trimutrate.png"))
dev.off()
...
write.csv(ref_mut_rate, paste0(cistype,"_mut_rate.csv") , row.names = FALSE, quote=FALSE)
