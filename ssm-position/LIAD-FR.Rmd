...
kable(table_exon_mutation_by_gene %>% dplyr::slice(1:20))
```
...
kable(table_promoter_mutation_by_gene %>% dplyr::slice(1:20))
```
...
kable(table_enhancer_mutation_by_gene %>% dplyr::slice(1:20))
```

## Regulatory mutations

The transcripts whose regulatory regions harbor the greatest number of mutations:
...
kable(table_regulatory_mutation_by_tss %>% arrange(desc(count)) %>% dplyr::slice(1:20))
```
...
kable(table_regulatory_mutation_by_gene %>% arrange(desc(count)) %>% dplyr::slice(1:20))
```
