...
for(tf_name in tf_names){
  ...
  p_value_filenames <- paste0("p_value_promoter_", upbm_filenames)
  mut_effect_filenames <- paste0("mut_effect_promoter_", upbm_filenames)
  num_files <- length(p_value_filenames)

  data_promoters_mutated_effect_list <- list()
  ...
  for(i in 1:num_files){
    ...
    data_promoters_mutated_effect_list[[i]] <- data_promoters_mutated_effect
  }

  names(data_promoters_mutated_effect_list) <- upbm_filenames
  print(data_promoters_mutated_effect_list)

  cor_plot_names <- unlist(lapply(strsplit(upbm_filenames, split = "|", fixed = T), "[", 4))

  if(length(data_promoters_mutated_effect_list) > 1){
    table_mut_effect <- matrix(0, nrow = length(promoters_w_mutation),
                               ncol = length(data_promoters_mutated_effect_list))
    for(j in 1:length(data_promoters_mutated_effect_list)){
      table_mut_effect[,j] <- data_promoters_mutated_effect_list[[j]]$mut_effect
    }
    colnames(table_mut_effect) <- cor_plot_names
    setwd("/Users/jz132/Desktop/hardac-xfer/plots")
    png(file = paste0(tf_name, "_promoter.png"), width = 1600, height = 1600, res = 160)
