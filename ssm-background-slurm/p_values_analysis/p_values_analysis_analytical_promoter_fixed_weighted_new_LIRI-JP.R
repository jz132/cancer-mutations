...
# import genomic coordinates of promoter and links between promoter and tss
...
# import promoter entropy for weighting
setwd(entropy.path)
...
# promoter not linked to any gene are excluded
...
upbm_filenames <- gsub(".*prediction", "prediction", jpred_promoter_filenames)
num_files <- length(jpred_promoter_filenames)

z_score_matrix_per_promoter <- NULL
...
for(i in 1:num_files){
  ...
  z_score_matrix_per_promoter <- cbind(z_score_matrix_per_promoter,
                                       data_promoter_mutated_effect$z_score)
  z_score_matrix_per_gene <- cbind(z_score_matrix_per_gene, 
                                   data_promoter_mutated_effect_summarised$z_score)
}

rownames(z_score_matrix_per_promoter) <- data_promoter_mutated_effect$promoter
colnames(z_score_matrix_per_promoter) <- upbm_filenames

rownames(z_score_matrix_per_gene) <- data_promoter_mutated_effect_summarised$gene
colnames(z_score_matrix_per_gene) <- upbm_filenames

p_value_matrix_per_promoter <- 2*(1-pnorm(abs(z_score_matrix_per_promoter)))
p_value_matrix_per_gene <- 2*(1-pnorm(abs(z_score_matrix_per_gene)))

# adjusted p value after correcting for multiple comparison
adjusted_p_value_matrix_per_promoter <- apply(p_value_matrix_per_promoter, 2,
...
z_score_absmax_per_promoter <- apply(z_score_matrix_per_promoter, 1, absmax)
p_value_min_per_promoter <- apply(p_value_matrix_per_promoter, 1, min)
adjusted_p_value_min_per_promoter <- apply(adjusted_p_value_matrix_per_promoter, 1, min)
upbm_which_min_per_promoter <- upbm_filenames[apply(p_value_matrix_per_promoter, 1, 
                                                    which.min)]
data_promoter_mutated_max_effect_per_promoter <- data_promoter_mutated_effect %>% 
  select(promoter, count) %>%
  mutate(absmax_z_score = z_score_absmax_per_promoter,
         min_p_value = p_value_min_per_promoter,
         min_adjusted_p_value = adjusted_p_value_min_per_promoter,
         upbm = upbm_which_min_per_promoter) %>%
  inner_join(mapping_table_per_upbm) %>%
  arrange(min_p_value)
hist(log10(data_promoter_mutated_max_effect_per_promoter$min_p_value))
summary(log10(data_promoter_mutated_max_effect_per_promoter$min_p_value))
...
upbm_which_min_per_gene <- upbm_filenames[apply(p_value_matrix_per_gene, 1, 
                                                which.min)]
data_promoter_mutated_max_effect_per_gene <- data_promoter_mutated_effect_summarised %>%
  select(gene, count) %>% 
  ...
  inner_join(mapping_table_per_upbm) %>%
  arrange(min_p_value)
hist(log10(data_promoter_mutated_max_effect_per_gene$min_p_value))
summary(log10(data_promoter_mutated_max_effect_per_gene$min_p_value))
...
data_promoter_result_per_gene <- data_promoter_mutated_max_effect_per_gene %>% 
  left_join(data_cosmic %>% select(gene, cosmic_tier))

setwd(output.path)
filename_output <- paste0(datasetname, "_promoter_result_brief.csv")
write.csv(data_promoter_result_per_gene, filename_output, row.names = F, quote = F)

# annotate each gene by expression data
setwd(expression.path)
if(length(Sys.glob(filename_icgc_exp)) == 0){
  data_promoter_result_per_gene <- data_promoter_result_per_gene %>%
    mutate(expr_avail = 0)
} else{
  ...
  for(gene_select in data_promoter_result_per_gene$gene){
    data_promoter_annotated_select <- data_promoter_annotated %>%
      filter(gene == gene_select)
    
    donor_select <- data_mutations_promoter %>%
      genome_inner_join(data_promoter_annotated_select) %>%
      pull(icgc_donor_id) %>%
      unique()
...
setwd(output.path)
filename_output <- paste0(datasetname, "_promoter_result_per_gene_weighted.csv")
write.csv(data_promoter_result_per_gene, filename_output, row.names = F, quote = F)

# expand the result to include all significant TFs for each top gene
p_cutoff <- 0.1
data_promoter_result_per_gene_top <- data_promoter_result_per_gene %>%
  filter(min_adjusted_p_value < p_cutoff)

top_genes <- data_promoter_result_per_gene_top %>% pull(gene)

data_promoter_result_per_gene_top_expanded <- NULL
for(gene in top_genes){
  ...
  temp_result <- tibble(gene = gene,
                        ...
                        upbm = upbm_select) %>%
    inner_join(data_promoter_result_per_gene_top %>%
  ...
  data_promoter_result_per_gene_top_expanded <- bind_rows(
    data_promoter_result_per_gene_top_expanded,
    temp_result)
}

setwd(output.path)
filename_output <- paste0(datasetname, "_promoter_result_weighted_top_gene_expanded.csv")
write.csv(data_promoter_result_per_gene_top_expanded,
          filename_output, row.names = F, quote = F)

# output the gene each mutation is associated with
data_mutations_promoter_annotated <- data_mutations_promoter %>% 
  genome_inner_join(data_promoter_annotated) %>%
  select(chromosome = chromosome.x,
         start = start.x,
         end = end.x,
...
setwd(output.path)
filename_output <- paste0(datasetname, "_promoter_mutations_annotated.csv")
write.csv(data_mutations_promoter_annotated,
          filename_output, row.names = F, quote = F)
