...
for(i in 1:n_enhancers_w_mutation){
  ...
  ## Step 3: Simulation ##
  n_mut_sim <- matrix(0, nrow = n_run, ncol = nrow(data_enhancer_mutation_prediction))
  for(j in 1:ncol(n_mut_sim)){
    n_mut_sim[,j] <- rbinom(n_run, num_donors, 
                            data_enhancer_mutation_prediction$tri_mut_rate)
  }
  ...
  for(k in 1:n_run){
    if(!all(n_mut_sim[k,] == 0))
      mut_effect_sim_mutations[k] <- 
        absmax(data_enhancer_mutation_prediction$diff[which(n_mut_sim[k,] != 0)])
  }
  
  ## Step 4: get the enhancer level mutation effect and p-value ##
  mut_effect_enhancer <- absmax(mut_effect_actual_mutations$diff)
  n_mut_enhancer <- nrow(mut_effect_actual_mutations)

  p_less <- sum(mut_effect_sim_mutations < mut_effect_enhancer)/n_run
