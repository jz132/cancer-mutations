...
set.seed(1234)
n_run <- 10^6
promoters_per_job <- 100
p_value_list <- rep(1, nrow(data_promoters_mutated))
promoters_w_mutation <- which(data_promoters_mutated$count > 0) # 24619
promoters_w_mutation_select <- promoters_w_mutation[((cmd_arg-1)*promoters_per_job+1):
                                                      min(cmd_arg*promoters_per_job, 
                                                          length(promoters_w_mutation))]

for(promoter_ex in promoters_w_mutation_select){
  print(promoter_ex)
...
setwd(output.path)
write.table(p_value_list,  paste0("p_value_list_variable_savemem_", 
                                  promoters_per_job, "_", cmd_arg, ".txt"), 
