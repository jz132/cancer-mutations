...
set.seed(1234)
n_run <- 10^6
enhancers_per_job <- 100
p_value_list <- rep(1, nrow(data_enhancers_mutated))
enhancers_w_mutation <- which(data_enhancers_mutated$count > 0)
enhancers_w_mutation_select <- enhancers_w_mutation[((cmd_arg-1)*enhancers_per_job+1):
                                                    min(cmd_arg*enhancers_per_job, 
                                                        length(enhancers_w_mutation))]

for(enhancer_ex in enhancers_w_mutation_select){
  print(enhancer_ex)
...
setwd(output.path)
write.table(p_value_list,  paste0("p_value_list_enhancer_variable_savemem_", 
                                  enhancers_per_job, "_", cmd_arg, ".txt"), 
