...
for(i in 1:n_promoters_w_mutation){
  ...
  ## Step 3: Simulation ##
  n_mut_sim <- matrix(0, nrow = n_run, ncol = nrow(data_promoter_mutation_prediction))
  for(j in 1:ncol(n_mut_sim)){
    n_mut_sim[,j] <- rbinom(n_run, num_donors, 
                            data_promoter_mutation_prediction$tri_mut_rate)
  }
  
  mut_effect_sim_mutations <- numeric(n_run)
  for(k in 1:n_run){
    if(!all(n_mut_sim[k,] == 0))
      mut_effect_sim_mutations[k] <- 
        absmax(data_promoter_mutation_prediction$diff[which(n_mut_sim[k,] != 0)])
  }
  
  ## Step 4: get the promoter level mutation effect and p-value ##
  mut_effect_promoter <- absmax(mut_effect_actual_mutations$diff)
  n_mut_promoter <- nrow(mut_effect_actual_mutations)
  
  p_less <- sum(mut_effect_sim_mutations < mut_effect_promoter)/n_run
